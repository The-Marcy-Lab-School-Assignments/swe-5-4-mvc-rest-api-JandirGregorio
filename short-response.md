...
**Your answer here**: 
1. The `handleTodosListClick()` function checks if the `toggle-btn` was pressed as event listener callback in the `frontend/src/main.js` file.
...
5. The controllers' `updateTodo()` method reads the response and calls the `update()` method from the `server/models/todoModel.js` file.
6. The `update()` method looks for the todo task with the specific `id` extracted from the route/path parameters, updates `isDone` to `true` or `false`, and returns the updated value to the controllers' `updateTodo` method.
